                                ...
                                float *maxEdgeEigenvalues) //OP_WRITE
{
  ...
  if (!*isRightBoundary) {
    ...
    float inNormalVelocity = leftCellValues[1] * nx + leftCellValues[2] * ny;
    float inTangentVelocity = -1.0f * leftCellValues[1] * ny + leftCellValues[2] * nx;
   ...
   //     int count = 1;
   //     while  ( double(count)*0.05 < *timestamp){
   //     //while  ( double(count)*30 < *timestamp){
   //      count = count + 1;
   //     }
   //     ifstream in;
   //     in.open("./volna2hdf5/monai/monai_input_edit.txt");
   //     //in.open("./volna2hdf5/hilo/incoming_wave.txt");
   //     float h_0;
   //     for (int i =0; i<count; i++){
   //         in >> h_0;
   //     }
   //     rightCellValues[0] = -1.0 * rightCellValues[3];
   //     rightCellValues[0] += h_0;
   //     outNormalVelocity = inNormalVelocity;
   //     outNormalVelocity += 2.0 * sqrt( g * leftCellValues[0] );
   //     outNormalVelocity -= 2.0 * sqrt( g * rightCellValues[0] );
   //     outTangentVelocity = inTangentVelocity;
   //     rightCellValues[1] = outNormalVelocity * nx - outTangentVelocity * ny;
   //     rightCellValues[2] = outNormalVelocity * ny + outTangentVelocity * nx;
   // } else if ((edgeCenters[0] == 0.0f) && (*timestamp > 22.5)){
   //     //Outflow
   //     rightCellValues[0] = -1.0f*leftCellValues[3];
   //     outTangentVelocity = inTangentVelocity;
   //     rightCellValues[1] = outNormalVelocity * nx - outTangentVelocity * ny;
  ...
  // Wetting and Drying (Brocchini et al. 2001)
  ...
  // If HLLC (Huang et al. 2013) Normal Momentum flux term
  //float LeftFluxes_N = HuDotN * uLn;
  ...
  // If HLLC (Huang et al. 2013) Normal Momentum flux term
  //float RightFluxes_N =   HuDotN * uRn;
  ...
  if( sL >= 0.0f) {
    ...
    UStarL = leftCellValues[0]*tStar;
    out[0] = LeftFluxes_H + sL*(UStarL-leftCellValues[0]);
    out[1] = LeftFluxes_U + sL*(UStarL*(leftCellValues[1]+ edgeNormals[0]*(sStar-uLn))-leftCellValues[0]*leftCellValues[1]);
    out[2] = LeftFluxes_V + sL*(UStarL*(leftCellValues[2]+ edgeNormals[1]*(sStar-uLn))-leftCellValues[0]*leftCellValues[2]);
  } else if((sStar < 0.0f) && (sR >= 0.0f)){
    float UStarR;
    float tStar = (sR - uRn)/(sR - sStar);
    UStarR = rightCellValues[0]*tStar;
    out[0] = RightFluxes_H + sR*(UStarR-rightCellValues[0]);
    out[1] = RightFluxes_U + sR*(UStarR*(rightCellValues[1]+ edgeNormals[0]*(sStar-uRn))-rightCellValues[0]*rightCellValues[1]);
    out[2] = RightFluxes_V + sR*(UStarR*(rightCellValues[2]+ edgeNormals[1]*(sStar-uRn))-rightCellValues[0]*rightCellValues[2]);
  } else {
    out[0] = RightFluxes_H;
    out[1] = RightFluxes_U;
    out[2] = RightFluxes_V;
