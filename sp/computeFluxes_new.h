                                ...
                                float *maxEdgeEigenvalues) //OP_WRITE
{
  ...
  */
  ...
  if (!*isRightBoundary) {
  leftCellValues[0] += ((dxl * leftGradient[0])+(dyl * leftGradient[1]));
  rightCellValues[0] += ((dxr * rightGradient[0])+(dyr * rightGradient[1]));
  ...
  leftCellValues[3] +=((dxl * leftGradient[6])+(dyl * leftGradient[7]));
  rightCellValues[3] += ((dxr * rightGradient[6])+(dyr * rightGradient[7]));
  
  leftCellValues[1] += ((dxl * leftGradient[2])+(dyl * leftGradient[3]));
  leftCellValues[2] += ((dxl * leftGradient[4])+(dyl * leftGradient[5]));

  rightCellValues[1] += ((dxr * rightGradient[2])+(dyr * rightGradient[3]));
  rightCellValues[2] += ((dxr * rightGradient[4])+(dyr * rightGradient[5]));
  
  }
  //op_printf(" Center %g %g Zl %g Z*l %g Gradient %g lim  \n", leftcellCenters[0], leftcellCenters[1], cellLeft[3], leftCellValues[3], leftGradient[6]); 
  ...
  out[1] *= *edgeLength;
  out[2] *= *edgeLength;
  //op_printf(" loc %g %g bathy %g %g out %g %g %g\n", leftcellCenters[0], leftcellCenters[1], bathySource[0], bathySource[2], out[0], out[1], out[2]); 
